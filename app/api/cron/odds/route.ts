...
export async function GET(req: NextRequest) {
  ...
  // Build a lookup keyed by sorted canonical team-name pair → match rows.
  ...
  for (const item of items) {
    ...
    const avg = averageH2H(item);
    if (!avg) continue;

    // home/away in our DB may be reversed vs odds-api; swap if needed.
    const reversed = target.m.home_team !== canonicalHome;
    const home_odds = reversed ? avg.away : avg.home;
    const away_odds = reversed ? avg.home : avg.away;
